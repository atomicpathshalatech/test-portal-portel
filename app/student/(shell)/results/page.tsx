...
export default async function ResultsHistoryPage() {
  ...
  const points = attempts.filter((a) => a.test).map((a) => {
    const test = a.test!;
    const questionCount = test.sections.reduce((s, sec) => s + sec.questions.length, 0);
    const maxMarks = questionCount * test.correctMarks;
    const pct = maxMarks > 0 ? ((a.score || 0) / maxMarks) * 100 : 0;
    return { pct, score: a.score ?? 0, name: test.name, date: a.submittedAt };
  });
