...
export default function UnifiedQuestionAuthoringPage() {
  ...
  const [importMode, setImportMode] = useState<"none" | "bank" | "previous">("none");
  const [bankQuestions, setBankQuestions] = useState<QuestionFull[]>([]);
  ...
  async function openBank() {
    if (!activeSection) return;
    setImportMode("bank");
    const res = await fetch(`/api/questions?subject=${encodeURIComponent(activeSection.subject)}`);
  ...
  async function importFromBank(q: QuestionFull) {
    if (!activeSection) return;
    await fetch(`/api/sections/${activeSection.id}/questions`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    <div className="fixed inset-0 flex bg-panel z-[100]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-6">
          ...
          {/* Auto-translate / check-translation toolbar */}
          {form.enableHi !== form.enableEn ? (
          ...
          {/* Side-by-side Hindi | English authoring — both edited simultaneously */}
        ...
        <div className="fixed inset-0 bg-black/40 z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-2 mt-4">
              <button
                type="button"
                onClick={() => router.push(`/admin/tests/${testId}/add-questions`)}
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-6">
          <div className="card w-full max-w-lg max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-1">
              {bankQuestions
                .filter((q) => !bankSearch || q.translations[0]?.statement?.toLowerCase().includes(bankSearch.toLowerCase()) || q.questionCode?.toLowerCase().includes(bankSearch.toLowerCase()))
                .map((q) => (
                  <div key={q.id} className="flex items-center justify-between py-2 border-b text-sm">
